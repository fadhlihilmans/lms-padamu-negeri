<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::create('absensi_detail', function (Blueprint $table) {
            $table->id();
            $table->foreignId('sesi_absensi_id')->constrained('sesi_absensi')->cascadeOnDelete();
            $table->foreignId('peserta_didik_id')->constrained('peserta_didik')->cascadeOnDelete();
            $table->enum('status', ['hadir', 'izin', 'sakit', 'alpa']);
            $table->dateTime('waktu_klik')->nullable();
            // diubah_manual_oleh → users; nullable
            $table->unsignedBigInteger('diubah_manual_oleh')->nullable();
            $table->foreign('diubah_manual_oleh')->references('id')->on('users')->nullOnDelete();
            $table->timestamps();

            $table->unique(['sesi_absensi_id', 'peserta_didik_id']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('absensi_detail');
    }
};
