        ...
        <div class="bg-white rounded-xl border border-[#c5c5d7] shadow-sm p-4 sm:p-6 mb-6">
            ...
            <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                @foreach ($lampiranGambar as $g)
                    <button type="button"
                            x-data
                            x-on:click="$dispatch('open-file-preview', { url: '{{ Storage::url($g->file_path) }}', name: @js($g->nama_asli ?? 'Gambar') })"
        ...
        <div class="bg-white rounded-xl border border-[#c5c5d7] shadow-sm p-4 sm:p-6 mb-6">
            ...
            <div class="flex flex-col gap-3">
                @foreach ($lampiranFiles as $f)
                    @php [$icon, $iconColor, $iconBg] = $f->ikonWarna(); @endphp
                    <div class="p-4 rounded-xl border border-[#c5c5d7] bg-[#f6fafe]">
                        ...
                        <div class="flex items-center gap-2 mt-3">
                            {{-- Preview untuk SEMUA tipe: PDF/gambar dirender, tipe lain
                                 tampil kartu "pratinjau tidak tersedia" + Unduh. --}}
                            <button type="button"
                                    x-data
                                    x-on:click="$dispatch('open-file-preview', { url: '{{ Storage::url($f->file_path) }}', name: @js($f->nama_asli ?? 'File') })"
