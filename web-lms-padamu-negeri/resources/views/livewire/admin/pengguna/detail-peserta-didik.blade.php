<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <a href="{{ route('admin.pengguna.peserta-didik') }}"
           class="inline-flex items-center gap-1.5 text-[13px] text-[#505f76] hover:text-[#3c50e0] transition-colors cursor-pointer">
            ...
            Kembali ke Manajemen Peserta Didik
        </a>
    </div>
    ...
    <div class="bg-white rounded-xl border border-[#c5c5d7] shadow-sm p-6 mb-6">
        <div class="flex flex-col sm:flex-row items-start gap-5">
            <div class="w-16 h-16 rounded-2xl bg-[#EEF2FF] flex items-center justify-center flex-shrink-0">
                <span class="material-symbols-outlined text-[#3c50e0] text-[32px]">person</span>
            </div>
            <div class="flex-1 min-w-0">
                <h1 class="text-[22px] font-bold text-on-surface mb-1">{{ $pesertaDidik->nama_lengkap }}</h1>
                <div class="flex flex-wrap items-center gap-3 text-[13px] text-[#505f76] mb-3">
                    ...
                    @if ($pesertaDidik->nisn)
                        ...
                        <span class="flex items-center gap-1">
                            <span class="material-symbols-outlined text-[14px]">numbers</span>
                            NISN: <span class="font-mono font-semibold text-on-surface ml-1">{{ $pesertaDidik->nisn }}</span>
                        </span>
                    @endif
                </div>
                <div class="flex flex-wrap gap-2">
                    ...
                    @endphp
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-[12px] font-semibold {{ $statusColors[$pesertaDidik->status_akademik] ?? 'bg-gray-100 text-gray-700' }}">
                        {{ ucfirst($pesertaDidik->status_akademik) }}
                    </span>
                    <span @class([
                        ...
                        Akun {{ $pesertaDidik->user?->is_active ? 'Aktif' : 'Nonaktif' }}
                    </span>
                </div>
            </div>
            <a href="{{ route('admin.pengguna.peserta-didik') }}"
        ...
        <div class="lg:col-span-2 flex flex-col gap-6">
            ...
            <div class="bg-white rounded-xl border border-[#c5c5d7] shadow-sm p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    @if ($pesertaDidik->tempat_lahir || $pesertaDidik->tanggal_lahir)
                        <div>
                            <p class="text-[12px] text-[#757686] mb-0.5">Tempat, Tanggal Lahir</p>
                            <p class="text-[14px] font-medium text-on-surface">
                                {{ $pesertaDidik->tempat_lahir ?? '—' }}
                                @if ($pesertaDidik->tanggal_lahir)
                                    , {{ $pesertaDidik->tanggal_lahir->format('d M Y') }}
                                @endif
                            </p>
                        </div>
                    @endif
                    @if ($pesertaDidik->agama)
                        <div>
                            <p class="text-[12px] text-[#757686] mb-0.5">Agama</p>
                            <p class="text-[14px] font-medium text-on-surface">{{ ucfirst($pesertaDidik->agama) }}</p>
                        </div>
                    @endif
                    @if ($pesertaDidik->nik)
                        <div>
