...
- Aset logo dikelola Admin melalui modul Pengaturan Aplikasi (lihat Bab 6.12),
  disimpan via `storage:link`, dirujuk oleh template PDF lewat `SettingService`.
  Lebih fleksibel dari draft awal yang mengasumsikan logo sebagai berkas statis
  tetap di `public/images/` — kini Admin dapat menggantinya sendiri.
- Tim pengembang tetap perlu meminta berkas logo resmi (PNG/SVG transparan) dari
  lembaga di awal, untuk diunggah pertama kali via modul Pengaturan (lihat Bab 10.5).

## 6.12 Pengaturan Aplikasi (Settings)

**Deskripsi.** Modul tambahan hasil iterasi PRD — menyediakan satu tempat bagi
Admin untuk mengubah konfigurasi global tanpa perlu deploy ulang kode. Hanya
Admin yang punya akses ke modul ini.

**Cakupan v1:**

- **Mode Maintenance** — toggle on/off. Saat aktif, seluruh akses non-Admin
  diarahkan ke halaman pemeliharaan; Admin tetap bisa login dan bekerja normal.
- **Identitas PKBM** — Nama PKBM (dipakai di sidebar, topbar, halaman login,
  dan kop rapor), Alamat, Telepon/WhatsApp, Email.
- **Aset Kop Rapor** — Logo Kabupaten dan Logo PKBM, diunggah lewat UI (bukan
  lagi murni aset statis `public/images/` seperti draft awal Bab 6.11.1 —
  Admin dapat menggantinya sendiri tanpa bantuan developer).
- **Batas Upload** — ukuran maksimal file Materi dan file submisi Tugas
  (dalam MB), divalidasi di form upload terkait.
- **Toggle Modul** — aktif/nonaktif untuk modul CBT, Tugas, dan Absensi
  (daftar dapat bertambah). Saat modul dinonaktifkan:
  - Menu modul tersebut hilang dari sidebar Guru & Peserta Didik (Admin tetap
    melihatnya untuk keperluan administratif).
  - **Akses langsung ke URL modul tersebut tetap diblokir di level
    route/middleware**, bukan hanya disembunyikan dari menu — mencegah akses
    lewat tautan langsung selama modul nonaktif.

**Implementasi Teknis:**

- Disimpan sebagai tabel key-value `settings` (lihat `database.md` Bagian 8),
  bukan file `.env` — supaya dapat diubah lewat UI tanpa deploy ulang dan tanpa
  migration baru saat menambah setting baru.
- Diakses lewat satu Service (`SettingService`) yang melakukan caching dan
  type-casting, dipakai konsisten di seluruh aplikasi (Middleware, Livewire,
  template Blade kop rapor).

## 6.13 Log Error Sistem

**Deskripsi.** Modul tambahan hasil iterasi PRD — mencatat otomatis setiap
Exception PHP (`\Throwable`) yang tertangkap di blok try-catch, supaya Admin
punya satu tempat untuk menelusuri error teknis tanpa perlu akses server log
langsung. Hanya Admin yang dapat melihat & mengelola log ini.

**Alur:**

- Setiap kali blok try-catch di aplikasi menangkap Exception, sistem otomatis
  menyimpan satu baris ke tabel `error_log`: aksi yang sedang dilakukan,
  pesan error, pengguna yang sedang login (jika ada), dan URL terkait.
- Pengguna yang mengalami error tetap melihat pesan ramah ("Maaf, terjadi
  kesalahan.") — detail teknis tidak ditampilkan ke pengguna, hanya tersimpan
  untuk Admin.
- Admin membuka menu Log Error untuk melihat daftar error terbaru, dapat
  melihat detail lengkap per baris.
- Admin dapat menghapus log satu per satu, atau menghapus seluruhnya
  sekaligus ("Hapus Semua") — log ini bersifat operasional jangka pendek,
  bukan arsip historis, sehingga tidak menggunakan soft delete.

## 6.14 Lapor Bug / Masalah

**Deskripsi.** Modul tambahan hasil iterasi PRD — memungkinkan **seluruh
pengguna** (Admin, Guru, maupun Peserta Didik) melaporkan masalah yang mereka
temui secara manual, berbeda dari Log Error Sistem yang otomatis. Berguna
untuk menangkap masalah yang tidak memicu Exception PHP, misalnya tampilan
rusak atau tombol yang terasa tidak berfungsi sesuai harapan.

**Alur Pelaporan (semua role):**

- Tombol "Lapor Bug" tersedia di topbar, dapat diakses dari halaman manapun.
- Pengguna mengisi judul singkat, deskripsi masalah, dan dapat melampirkan
  screenshot (opsional).
- Sistem otomatis mencatat siapa yang melapor dan URL halaman saat laporan
  dikirim.
- Laporan masuk dengan status awal "Baru".

**Alur Tindak Lanjut (Admin):**

- Admin melihat seluruh laporan yang masuk, dapat memfilter berdasarkan
  status (Baru, Diproses, Selesai, Ditolak).
- Admin mengubah status laporan dan dapat menambahkan catatan tindak lanjut.
- Admin dapat menghapus laporan yang sudah selesai ditangani — tidak
  menggunakan soft delete, sama seperti Log Error Sistem.
...
- Logo Kabupaten Batang dan logo PKBM untuk kop rapor dikelola lewat modul
  Pengaturan Aplikasi (Bab 6.12) — Admin dapat mengganti logo langsung dari UI
  tanpa bantuan developer maupun deployment ulang.
...
- `app/Livewire/Admin/Pengaturan/` — SettingManager.php, GradeManager.php
- `app/Livewire/Admin/ErrorLog/` — DaftarErrorLog.php
- `app/Livewire/BugReport/` — FormLaporBug.php (semua role), DaftarBugReport.php (Admin)
...
11. Pengaturan Aplikasi (Settings, Grade, Toggle Modul, Maintenance Mode)
12. Log Error & Lapor Bug
